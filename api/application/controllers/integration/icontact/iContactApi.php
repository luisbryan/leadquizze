...
class iContactApi {
	...
	**/
	protected function __construct() {
		...
		$aConstantMap = array(
			...
			'ICONTACT_APPID'       => 'ZKWj2qaVbvMUR0ClbYK6Z286soYpbB5T', 
			// 'ICONTACT_APIURL', 
			'ICONTACT_APIUSERNAME' => 'justformenbrand@gmail.com',  
			'ICONTACT_APIPASSWORD' => 'livejustformen@2013'
		);
                
              /*  
               * =======================for icontact sandbox jfm=====================
               * $aConstantMap = array(
			// 'ICONTACT_APIVERSION', 
			// 'ICONTACT_APISANDBOXURL', 
			'ICONTACT_APPID'       => '4siBy2ZKJ9W4xmXB9HDTtWS0vqDZpgwj', 
			// 'ICONTACT_APIURL', 
			'ICONTACT_APIUSERNAME' => 'sfranzken-beta',  
			'ICONTACT_APIPASSWORD' => 'synchpass'
		); */
	 ...
	 * @return object
	**/
	public function addContact( $arrValues = array() ) {
		
		//$arrParam:  email, prefix, suffix, firstName, lastName, street, street2, city, state, postalCode, phone, fax, business 
		// Valid statuses
		...
		if(!empty( $arrValues ))
		{
			foreach(  $arrValues as $key => $RV)
			{
				if( $RV != null && $RV != '' )
				{
					if( 'status' == $key &&  in_array($RV, $aValidStatuses) )
					{
						$aContact['status'] = (string) $RV;
					}
					else
					{
						$aContact[$key] = (string)$RV;
					}
				}
			}
	...
	**/
	 ...
	 * @return bool|object
	 **/
	public function updateContact($iContactId,  $arrValues = array() ) {
		
		/* Default keys (contactId, email, prefix, firstName, lastName,  suffix, street, street2,  city, state,  postalCode, status, phone, fax, business  */
		// Valid statuses
		$aValidStatuses = array('normal', 'bounced', 'donotcontact', 'pending', 'invitable', 'deleted', );
		// Contact placeholder
		$aContact       = array();
...
//		// Make the call and return the data
//		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts/?email={$email}&status=total", 'GET', null, 'contact');
//	}
	/**
	...
	**/
	public function getContacts($limit = 20, $offset=0) {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts?status=total&limit=".$limit."&offset=".$offset, 'GET');
	}
