function scrollToError() {
  setTimeout( function(){
    if( $('.has-error:first').length > 0 ) {
      $('html,body').animate({scrollTop: $('.has-error:first').parent().offset().top}, 400);
    }
    if( $('.alert-danger').length > 0 ) {
      $('html,body').animate({scrollTop: $('.alert-danger').offset().top}, 400);
    }
  }, 500 );
}
...
function checkAnswers() {
    $('.answer').each( function(){
      if( $(this).val() == '' ) {
        $(this).parent().parent().css('border-color', '#ea4570').click( function() {
          $(this).css('border-color', 'rgb(157, 163, 168)');
        });
      }
    });
}
  ...
  $('#new-admin').show();
   ...
   $('#datatable').on('click', 'input[type="checkbox"]', function(e){
      ...
      if( $('tr.selected').length == 0 ) {
        $('#action-no-quiz').show();$('#new-admin').show();
        ...
        $('#action-delete').show();
        $('#action-publish').show();
        $('#action-unpublish').show();
        $('#action-export').show();

        $('#action-suspend').show();
        $('#action-activate').show();
      } else {
        $('#action-archive').show();
        $('#action-delete').show();
        $('#action-publish').show();
...
function initTiles() {
      ...
      $('body').on( 'click', '.quiz-card', function() {
      $('#actions li').hide();
